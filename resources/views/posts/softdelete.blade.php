...
@endsection
@else
@section('heading','soft delete سله المهملات')
@endif
@section('content')
...
<a href="{{ route('restore',$post['id']) }}">ارجاع</a></button>
<form style="display: inline;" action="{{ route('forceDelete',$post['id']) }}" method="get">
  @method('GET')
  @csrf
             <input type="submit" value="حزف دائم" class="focus:outline-none text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-900"></button>
</form>
</td>
            </tr>
          @endforeach
